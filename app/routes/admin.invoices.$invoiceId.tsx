...
export default function Invoice() {
  ...
  return (
    <>
      <AbandonInvoiceModal invoiceId={invoice.id} open={openAbandon} setOpen={setOpenAbandon} />
      <main className="flex h-full flex-col pb-24">
        <section className="border-b border-gray-200 pb-4">
          <div className="flex flex-wrap justify-between border-b border-gray-200 pb-8">
            <h1>
              Create Invoice <span className="text-xl text-gray-400">for {invoice.vendor.name}</span>
            </h1>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {actions.map(({ href, icon, title }) => {
              const isDisabled = href !== "ticket" && invoice.tickets.length === 0;
              return (
                <ButtonNavLink
                  key={href}
                  to={href}
        ...
        <section className="mt-4 pb-8">
          ...
          <ul className="flex flex-col space-y-4">
            {invoice.tickets.map((ticket) => {
              const chargeCount = invoice.charges.filter((c) => c.ticketId === ticket.id).length;
              return (
                <div className="flex items-center gap-4" key={ticket.id}>
                  <Disclosure
                    as="li"
                    className="w-full rounded-md border-l border-r border-t border-gray-200 bg-white"
                    defaultOpen={false}
                  >
                    {({ open }) => (
                      <>
                        <div className="flex items-center">
                          ...
                          >
                            <div className="text-left">
                              <span className="block font-medium">Ticket {ticket.id}</span>
                            ...
                            {chargeCount > 0 && (
                              <IconChevronRight
                                size={20}
                                className={
                                  open ? "rotate-90 transform transition duration-75" : "transition duration-75"
                                }
                              />
                            )}
                          </Disclosure.Button>
                            ...
                            {invoice.charges
                              ...
                              .map((charge) => (
                                ...
                                >
                                  ...
                                  <div className="text-left">
                                    <span>{charge.part?.name ?? charge.description}</span>
                                  </div>
                                  <div className="flex items-center gap-4 place-self-end">
                                    {charge.warrantyCovered && <span className="font-bold text-cyan-700/75">IW</span>}
                                    <span>{formatCurrency(charge.actualCost)}</span>
                                    <fetcher.Form method="post">
                                      <input type="hidden" name="chargeId" value={charge.id} />
                                      <TrashButton type="submit" name="_action" value="deleteCharge" disabled={busy}>
        ...
        <fetcher.Form className="space-y-2 pb-24 pt-8" method="post">
          ...
          <Button type="submit" disabled={busy} name="_action" value="finishInvoice">
            Finish Invoice
          </Button>
        </fetcher.Form>
      </main>
