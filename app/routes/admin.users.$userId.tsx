...
import { badRequest, getBusyState, notFoundResponse, useUser } from "~/utils/utils";
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  const { userId, email, firstName, lastName, role, campusRole, campusId } = result.data;
...
export default function NewUser() {
  ...
  const [campusId, setCampusId] = useState<string>(user.campusUserRole?.campusId ?? "");
  const [campusRole, setCampusRole] = useState<string>(user.campusUserRole?.role ?? "");
  ...
  return (
    <>
      ...
      <fetcher.Form method="post" action="/admin/users/send-password-reset">
        ...
        <Button type="submit" variant="secondary" disabled={fetcherBusy} className="mt-2">
      ...
      <Form className="mt-4 sm:max-w-[16rem]" method="post">
        ...
        <div className="space-y-4">
          ...
          {currentUser.role === "ADMIN" && (
            <Select label="Permissions" name="role" defaultValue={user.role} required>
